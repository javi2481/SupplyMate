...
import { COVERAGE_ORDER, type CoverageBand, type UiSlice } from "@/lib/scope";
...
export function applySuggestedFilter(chip: SuggestedChip, slice: UiSlice): ApplyResult {
  ...
  switch (chip.action) {
    ...
    case "filter_health": {
      ...
      const tag = bucket === "stockout_risk" ? "riesgo_quiebre" : bucket === "overstock" ? "sobrestock" : null;
      if (!tag) return { type: "noop" };
      return { type: "slice", slice: { ...slice, health: union(slice.health, tag) } };
    }
