...
async def test_classify_intent_parses_model_output():
    ...
    with (
        patch("app.agent.get_model", return_value="mock-model"),
        patch("app.intent_classifier.Runner.run", new=AsyncMock(return_value=Result())),
    ):
...
async def test_classify_intent_unavailable_returns_none():
    with patch("app.agent.get_model", side_effect=RuntimeError("no key")):
