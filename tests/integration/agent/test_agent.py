...
@pytest.mark.asyncio
async def test_rules_first_interprets_rexona_horizon(monkeypatch):
    """Brand + horizon free-text is owned by rules; LLM must not run."""
    ...
    assert response.scope.horizon_days == 14
    assert response.purchase_list
