...
- [`app/services/catalog_service.py`](../app/services/catalog_service.py) exposes unified row (`ProductMaster`) and deterministic recommendation.
- Fields min/max/reorder_point and prices are **context**; order quantity uses the 7-day formula in [`app/replenishment.py`](../app/replenishment.py).
